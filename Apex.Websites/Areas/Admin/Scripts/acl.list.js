(function ($) {
    ...
    function checkboxChangedHandler() {
        ...
        if (this.checked) {
            $deleteButton.removeClass("disabled").prop("disabled", false);
            setSubCheckboxesStatus(this.value, true);
            ...
            setSubCheckboxesStatus(this.value, false);
    ...
    function setSubCheckboxesStatus(parentId, checked) {
        var subCheckboxes = $table.find('[data-parentid="' + parentId + '"]');

        if (subCheckboxes.length > 0) {
            $.each(subCheckboxes, function (index, checkbox) {
                checkbox.checked = checked;
                setSubCheckboxesStatus(checkbox.value, checked);
            });
        }
    }
